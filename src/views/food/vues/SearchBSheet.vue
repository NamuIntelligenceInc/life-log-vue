<template>
  <div class="search-food-selection fixed-top bg-white" :style="{minHeight: `${screenHeight}px`}" :class="{'show': show}">
      ...
      <div class="container search-results" :style="{height: `${scrollHeight}px`, overflowY: 'auto'}">
        <div class="row">
          <div class="col-md-6 ml-auto mr-auto p-1">            
            ...
            <div v-else>
              <!-- <div v-if="latestFoods.length > 0">
                <div class="pl-3">
                  <strong>최근 등록하신 메뉴</strong>
                </div>
                <div class="list-group">
                  <a href="javascript:;" class="list-group-item list-group-item-action text-center" v-for="(item, index) of latestFoods" :key="index" @click="onClickSelectItem(item)">
                    {{ item }}
                  </a>      
                </div>
              </div>               -->
      ...
      <div class="fixed-bottom bg-white" style="height: 87px;" v-if="Object.keys(selectedFoods).length > 0">
        <div class="container">
          <div class="row">
            <div class="col-md-6 ml-auto mr-auto p-0">
              <div class="text-center pb-1 pt-1">
                <small v-if="!isPreventSelect">
                  <span class="text-primary">{{ selectableCount - Object.keys(selectedFoods).length }}</span>개 더 선택가능합니다
                </small>
                <small v-else>
